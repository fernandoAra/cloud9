...
| --- | --- |
| **Core Requirements & Functionality** | Chrome `/voice` takes microphone transcript into [question detection](apps/web/src/lib/uncertainty.ts), calls [Exa](apps/web/src/app/api/search/route.ts) in the background, and displays a sourced finding in the [activity panel](apps/web/src/app/voice/page.tsx). The microphone transcript and live findings have been observed with the team's keys. A full spoken offer has not yet been confirmed by a person. |
...
- Live Chrome rehearsal: the selected built-in microphone produced transcript text; the English research question appeared and live Exa findings were reported in the panel. Exa and Gemini routes were also exercised with the team's credentials. Human confirmation of audible playback is still pending as of this note. Do not present spoken delivery as verified until it is heard in a full rehearsal.
